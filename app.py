...
if df.empty:
    ...
    if filtered_df.empty:
        ...
        # Localizar o índice da coluna chamada 'Placa' (case-insensitive) e fatiar o DataFrame
        placa_idx = next((idx for idx, col in enumerate(ordered_df.columns) if col.lower() == 'placa'), None)
        if placa_idx is not None:
            ordered_df = ordered_df.iloc[:, :placa_idx]
